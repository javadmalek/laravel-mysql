...
class CompanyCatalogController extends Controller
{
    ...
    {
        $catalogs = CompanyCatalog::paginate(10);
        return View::make('catalogs.supplier.index', ['catalogs' => $catalogs, 'filter_key' => '']);
    }
    ...
    public function filter()
    {
        ...
        if ($filter_key) {
            ...
            if ($userType == 'SUPPLIER') {
                $catalogs = CompanyCatalog::paginate(10);
                return View::make('catalogs.supplier.index', ['catalogs' => $catalogs, 'filter_key' => $filter_key]);
    ...
    {
        return View::make('catalogs.supplier.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($userType == 'SUPPLIER') {
            ...
            // process the login
            if ($validator->fails()) {
                return Redirect::to('/supplier/companies/catalogs/create')
                ...
                if (Input::hasFile('logo')) {
                    ...
                    return Redirect::to('supplier/companies/catalogs/' . $catalog->id . '/edit')
                        ->withErrors('No file is Selected');

                $catalog->save();

                Session::flash('message', 'Successfully created Catalog!');
                return Redirect::to('supplier/companies/catalogs/');
            }
        }
        return response()->view('errors.403');
    }
    ...
    public function edit($id)
    {
        ...
        if ($catalog)
            return View::make('catalogs.supplier.edit')
    ...
    public function update(Request $request, $id)
    {
        ...
        // process the login
        if ($validator->fails()) {
            return Redirect::to('/supplier/companies/catalogs/' . $id . '/edit')
                ->withErrors($validator);
        } else {
            $catalog = new CompanyCatalog();
